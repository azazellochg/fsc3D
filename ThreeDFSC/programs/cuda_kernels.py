...
def calcNeighborsKernel(points_array,
    ...
    if float32(point[0]) < float32(highpassfilter):
        ...
        twentysix_neighboring_points_dist[0] = calculate_distance(float32(x - 1), float32(y), float32(z), center[0],
                                                                  center[1], center[2])
        twentysix_neighboring_points_dist[1] = calculate_distance(float32(x), float32(y - 1), float32(z), center[0],
                                                                  center[1], center[2])
        twentysix_neighboring_points_dist[2] = calculate_distance(float32(x), float32(y), float32(z - 1), center[0],
                                                                  center[1], center[2])
        twentysix_neighboring_points_dist[3] = calculate_distance(float32(x - 1), float32(y - 1), float32(z), center[0],
                                                                  center[1], center[2])
        twentysix_neighboring_points_dist[4] = calculate_distance(float32(x - 1), float32(y), float32(z - 1), center[0],
                                                                  center[1], center[2])
        twentysix_neighboring_points_dist[5] = calculate_distance(float32(x), float32(y - 1), float32(z - 1), center[0],
                                                                  center[1], center[2])
        twentysix_neighboring_points_dist[6] = calculate_distance(float32(x - 1), float32(y - 1), float32(z - 1),
                                                                  center[0], center[1], center[2])

        twentysix_neighboring_points_dist[7] = calculate_distance(float32(x + 1), float32(y), float32(z), center[0],
                                                                  center[1], center[2])
        twentysix_neighboring_points_dist[8] = calculate_distance(float32(x), float32(y + 1), float32(z), center[0],
                                                                  center[1], center[2])
        twentysix_neighboring_points_dist[9] = calculate_distance(float32(x), float32(y), float32(z + 1), center[0],
                                                                  center[1], center[2])
        twentysix_neighboring_points_dist[10] = calculate_distance(float32(x + 1), float32(y + 1), float32(z),
                                                                   center[0], center[1], center[2])
        twentysix_neighboring_points_dist[11] = calculate_distance(float32(x + 1), float32(y), float32(z + 1),
                                                                   center[0], center[1], center[2])
        twentysix_neighboring_points_dist[12] = calculate_distance(float32(x), float32(y + 1), float32(z + 1),
                                                                   center[0], center[1], center[2])
        twentysix_neighboring_points_dist[13] = calculate_distance(float32(x + 1), float32(y + 1), float32(z + 1),
                                                                   center[0], center[1], center[2])

        twentysix_neighboring_points_dist[14] = calculate_distance(float32(x + 1), float32(y - 1), float32(z),
                                                                   center[0], center[1], center[2])
        twentysix_neighboring_points_dist[15] = calculate_distance(float32(x + 1), float32(y), float32(z - 1),
                                                                   center[0], center[1], center[2])
        twentysix_neighboring_points_dist[16] = calculate_distance(float32(x + 1), float32(y - 1), float32(z - 1),
                                                                   center[0], center[1], center[2])
        twentysix_neighboring_points_dist[17] = calculate_distance(float32(x - 1), float32(y + 1), float32(z),
                                                                   center[0], center[1], center[2])
        twentysix_neighboring_points_dist[18] = calculate_distance(float32(x), float32(y + 1), float32(z - 1),
                                                                   center[0], center[1], center[2])
        twentysix_neighboring_points_dist[19] = calculate_distance(float32(x - 1), float32(y + 1), float32(z - 1),
                                                                   center[0], center[1], center[2])
        twentysix_neighboring_points_dist[20] = calculate_distance(float32(x - 1), float32(y), float32(z + 1),
                                                                   center[0], center[1], center[2])
        twentysix_neighboring_points_dist[21] = calculate_distance(float32(x), float32(y - 1), float32(z + 1),
                                                                   center[0], center[1], center[2])
        twentysix_neighboring_points_dist[22] = calculate_distance(float32(x - 1), float32(y - 1), float32(z + 1),
                                                                   center[0], center[1], center[2])

        twentysix_neighboring_points_dist[23] = calculate_distance(float32(x + 1), float32(y + 1), float32(z - 1),
                                                                   center[0], center[1], center[2])
        twentysix_neighboring_points_dist[24] = calculate_distance(float32(x + 1), float32(y - 1), float32(z + 1),
                                                                   center[0], center[1], center[2])
        twentysix_neighboring_points_dist[25] = calculate_distance(float32(x - 1), float32(y + 1), float32(z + 1),
                                                                   center[0], center[1], center[2])
...
def calcNeighbors(points_array, center, cutoff_fsc, cutoff_binarize, highpassfilter, min_cutoff, inputmrc,
                  memory_inmrc_thresholded, memory_inmrc_thresholdedbinarized, outarraythresholded,
                  outarraythresholdedbinarized):
    ...
    cuda.synchronize()
    calcNeighborsKernel[blockspergrid, threadsperblock](g_points_array, c, cutoff_fsc, cutoff_binarize, highpassfilter,
                                                        min_cutoff, g_inputmrc, g_memory_inmrc_thresholded,
                                                        g_memory_inmrc_thresholdedbinarized, g_outarraythresholded,
...
def calcDistanceKernel(boxsize, center, out):
    ...
    blockId = cuda.blockIdx.x + cuda.blockIdx.y * cuda.gridDim.x + cuda.gridDim.x * cuda.gridDim.y * cuda.blockIdx.z
    ...
    out[idx, 0] = math.sqrt(
        (out[idx, 1] - numba.float32(center[0])) ** 2 + (out[idx, 2] - numba.float32(center[1])) ** 2 + (
                out[idx, 3] - numba.float32(center[2])) ** 2)
...
def calcDistance(boxsize, center):
    ...
    start_time = time.time()
    blockspergrid = (math.ceil(boxsize / threadsperblock[0]), math.ceil(boxsize / threadsperblock[1]),
...
def sum_rows(NumAtROutPre_global_mem, Start, End):
    threadsperblock = (32, 1, 1)
...
@cuda.jit
def sum_rowsKernel(NumAtROutPre_global_mem,
...
# CUDA kernel
@cuda.jit()
def cuda_calcProd11(kXofR_global_mem, kYofR_global_mem, kZofR_global_mem, Prod11, NumOnSurf, r):
